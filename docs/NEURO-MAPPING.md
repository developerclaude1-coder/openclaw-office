...
**The ETTZ side** is cited from the owner's session brief of 2026-09-01, which states the inventory was verified live
on the estate that day. This research session ran in a cloud container attached to a git repository, with **no wire
access to the estate** — no Mac mini, no memory spine, no NAS. Nothing on the ETTZ side was independently re-verified
here, and no verification commands were run; where the brief distinguishes *live* from *planned* (notably the
CUSTODIAN A-002 interlock: **planned, not shipped** — two disposal batches have already run without it), that
distinction is preserved and load-bearing. Anyone acting on this document should re-verify the ETTZ column on the
wire first.
...
|---|---|---|---|
| 1 | **Growth-cone exploration** — filopodia emitted *before* a target is known: cheap, TTL'd (~9.5 min), majority-fail, spatially bounded, steered by cue gradients read differentially with adaptive gain; stabilized only on a confirming handshake (Ziv & Smith 1996; Bentley & Toroian-Raymond 1986; Rosoff 2004; Lohmann & Bonhoeffer 2008) | watch-contracts (a diff over *already-published* contracts — reconciliation, not exploration); DESIDERATA + IGNORANCE GRAPH (a static want-list, not a readable gradient); confirming envelopes (generic ack plumbing) | **missing** — nothing reaches; everything listens |
| 2 | **Microglial pruning** — self-written eat-me mark (PS) → complement tag (C1q→C3→iC3b) → one scoped executor (CR3); use-dependence carried by an activity-renewed *don't-eat-me veto* at the act site (CD47/SIRPα); competence time-boxed (P5→P9); the same contact also *induces* filopodia (Stevens 2007; Schafer 2012; Lehrman 2018; Weinhard 2018) | ENTROPY A-003 (dwell clocks, grace, heal-before-harm) + QUARANTINE (.why.md, owner-gated) + planned A-002 veto socket + BEATS surveillance; bank usage balances exist but are **unwired** to the disposer | **partial** — mark/execute separation, dwell-escalation and heal-before-harm live; use signal stranded; veto specced-not-live; no competence window; no constructive output; executor deliberately different |
...
| 4 | **Hebbian plasticity / LTP-LTD** — correlation-gated (NMDA AND-gate), synapse-local, bidirectional, non-destructive weight updates; stabilized by BCM sliding threshold + multiplicative homeostatic scaling (Nowak 1984; Dudek & Bear 1992; Bi & Poo 1998; Turrigiano 1998) | Procedures' worked→verified / failed→contested and claims' trials **are** two-factor, use-AND-outcome, item-local, bidirectional, actuating weights. The BANK's usage balances are **not** Hebbian: use-only, no outcome coupling, no actuation, no LTD, no normalization anywhere | **partial** |
...
where A-002 is specced; and biology treats **absence of tonic supply as the death signal** (axon survival is a lease
...
- **The zero-fire fact.** 71,225 episodic envelopes against 4 claims and 12 procedures — every one hand-seeded. The
...
separating silent orderly disposal from systemic damage. ETTZ's A-002 interlock ("does a verified snapshot hold
this?") is precisely that acknowledgment — and it is **specced, not live**: two quarantine batches were disposed of
while the first Time Machine copy stood at 26%. By the estate's own jurisprudence (the claims class is protected by a
guard *test*, not an intention), the mechanism does not exist until the disposal verb refuses without a custodian
envelope. This is the project's only live irreversible-loss exposure and ranks first in
[ADOPTABLE.md](./ADOPTABLE.md).
...
itself. Desiderata and the ignorance graph are a want-list, not a gradient (no differential "closer/farther" readout,
...
the disposer — PS is upstream of complement, Scott-Hewitt 2020, PMID:32657463).
**Breaks:** the use signal is stranded (bank balances derived from 71k envelopes; kallimachos doesn't read them — two
organs hold the two halves of the mechanism with no path between); polarity is inverted (accruing clock = positive
...
**Flagged as superficial:** `.why.md` manifests are *receipts written by the disposer at staging time*, not
pre-decision, item-local, instruction-free tags (iC3b). ETTZ has no "flagged, at T, by S, because Z" annotation that
...
*activity*, not silence (900 low-frequency pulses — Dudek & Bear 1992, PMID:1350090).
**Breaks:** the owner's named carrier — bank usage balances — is *not* Hebbian: use-only (strengthens a prompt that is
...
*retention* with *consolidation*: the selection problem (which of 71,225 weak traces deserve promotion into the stores
...
the 60 s+ cold fold is precisely the pathology CLS predicts for monolithic re-derivation instead of incremental
interleaved re-presentation.
**Breaks:** memory-nightly is an aggregation, not a replay — it renders state, mints no predicate, adds no trial;
the M1 harness re-runs episodes with plasticity disabled (outcomes never land as trials). No packet shape (biology:
...
pattern-completion, no generalization, and the 60 s+ fold. This is doctrine (the law forces the flight recorder) and
...
ablation performed by accident; durability rests on a backup that stood at 26%). Content-hash addressing is
...
graded recall (BINNA's union is boolean; corroboration raises nothing); no read-time arbitration (the 60 s+ fold is
...
**The break is §3 above:** authorization is not custody. The owner-gate verifies *permission*; Nagata's discriminator
is *completed content transfer*. Missing: the enforced handoff (disposal blocking on a custodian confirming envelope,
by content hash), a guard test on the disposal path (so the interlock cannot regress silently), snapshot pinning
...
until the owner rules, and accepts the throughput cost (2 batches/6 weeks vs 10¹¹ deaths/day) as the price of
accountability.
...
threshold is a static constant); homeostatic renormalization (bank balances rise monotonically forever —
